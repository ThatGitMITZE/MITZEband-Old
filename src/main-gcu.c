...
/*   _max_color = MAX(16, MIN(MAX_COLOR, COLORS));*/
   ...
   /* Parse Args and Prepare the Terminals. Rectangles are specified
      ...
      Examples:
        MITZEband -mgcu -- -right 30x27,* -bottom *x7 will layout as

        Term-0: Map (COLS-30)x(LINES-7) | Term-1: 30x27
        --------------------------------|----------------------
        <----Term-3: (COLS-30)x7------->| Term-2: 30x(LINES-27)

        MITZEband -mgcu -- -bottom *x7 -right 30x27,* will layout as
    ...
    */
    {
        ...
        if (remaining.cx < MAP_MIN_CX || remaining.cy < MAP_MIN_CY)
            quit(format("Failed: MITZEband needs an %dx%d map screen, not %dx%d", MAP_MIN_CX, MAP_MIN_CY, remaining.cx, remaining.cy));
        data[0].r = remaining;
